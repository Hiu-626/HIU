...
import { 
  ...
  Trophy, Sparkles, FileText, Target, ChevronRight, Activity, Shield, Layers, RefreshCcw, Info, X, DollarSign, Percent, Calendar, Languages, Coffee
} from 'lucide-react';
...
const I18N = {
  ...
  en: {
    ...
    freedomDesc: 'Covers est. expense ($15,000)',
...
const Insights: React.FC<InsightsProps> = ({ accounts, fixedDeposits, history, wealthGoal, onUpdateGoal }) => {
  ...
  const passiveData = useMemo(() => {
    ...
    // 財務自由度預計支出 (假設每月 15,000 HKD)
    const freedomProgress = Math.min(100, (totalMonthly / 15000) * 100);
    ...
    <div className="p-6 pb-28 space-y-8 bg-gray-50/50 min-h-screen font-sans animate-in fade-in duration-500 max-w-md mx-auto">
      ...
      <div className="flex justify-between items-start">
        <div>
           ...
           <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">{t.realTime}</p>
        </div>
        <div className="flex gap-2">
            <button onClick={() => setLang(lang === 'zh' ? 'en' : 'zh')} className="bg-white border border-gray-100 p-2.5 rounded-xl shadow-sm text-gray-500 hover:bg-gray-50 transition-colors">
                <Languages className="w-4 h-4" />
            </button>
            <button onClick={() => setShowReport(true)} className="bg-white border border-gray-100 px-4 py-2.5 rounded-xl shadow-sm text-blue-600 hover:scale-105 transition-transform active:scale-95 flex items-center gap-2 font-bold text-xs">
                <FileText className="w-4 h-4" /> {t.report}
            </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-[3rem] p-7 shadow-2xl text-white relative overflow-hidden">
            ...
            <div className="flex items-end gap-2 mb-6"><div className="text-4xl font-black tracking-tighter"><CountUp end={passiveData.totalMonthly} prefix="$" /></div><span className="text-sm font-bold text-gray-400 mb-1.5">{t.perMonth}</span></div>
            <div className="grid grid-cols-2 gap-3">
                <button onClick={() => setActiveIncomeDetail('FD')} className="bg-white/5 rounded-2xl p-4 backdrop-blur-md hover:bg-white/10 transition-all active:scale-95 text-left group border border-white/5">
                    ...
                    <div className="font-black text-white text-lg">${Math.round(passiveData.totalFDMonthly).toLocaleString()}</div>
                </button>
                <button onClick={() => setActiveIncomeDetail('STOCK')} className="bg-white/5 rounded-2xl p-4 backdrop-blur-md hover:bg-white/10 transition-all active:scale-95 text-left group border border-white/5">
                    ...
                    <div className="font-black text-white text-lg">${Math.round(passiveData.totalStockMonthly).toLocaleString()}</div>
                </button>
            </div>
            <div className="mt-4 pt-4 border-t border-white/5 flex items-center justify-between">
                <div className="text-[9px] text-gray-500 font-bold">{t.freedomDesc}</div>
      ...
      <div onClick={() => setIsEditingGoal(true)} className="bg-white border border-gray-100 rounded-[2.5rem] p-6 shadow-sm cursor-pointer active:scale-95 transition-transform">
          <div className="flex justify-between items-start mb-4">
             <div><h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1 flex items-center gap-2"><Target className="w-4 h-4" /> {t.goal}</h3><div className="text-2xl font-black text-gray-900 tracking-tight">${wealthGoal.toLocaleString()}</div></div>
          ...
          <div className="flex justify-between mt-2 text-[10px] font-bold text-gray-400 uppercase"><span>Current: ${passiveData.netWorth.toLocaleString()}</span><span className="text-[#0052CC]">{goalPercent.toFixed(1)}% {t.reached}</span></div>
      </div>
      ...
      {activeIncomeDetail && (
          <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center sm:px-4">
              ...
              <div className="bg-white w-full max-w-sm rounded-t-[3rem] sm:rounded-[3rem] p-8 pb-12 sm:pb-8 relative z-10 animate-in slide-in-from-bottom-20 max-h-[90vh] overflow-y-auto">
                  ...
                  <div className={`p-5 rounded-[2rem] mb-6 flex justify-between items-center ${activeIncomeDetail === 'FD' ? 'bg-yellow-50 text-yellow-800' : 'bg-blue-50 text-blue-800'}`}>
                      ...
                      <div className="text-right"><div className="text-[9px] uppercase font-black opacity-60 mb-1">{t.passiveIncome}</div><div className="text-xl font-black">${Math.round(activeIncomeDetail === 'FD' ? passiveData.totalFDMonthly : passiveData.totalStockMonthly).toLocaleString()}</div></div>
                  </div>
                      ...
                      {(activeIncomeDetail === 'FD' ? passiveData.fdDetails : passiveData.stockDetails).map((item, idx) => (
                          <div key={item.id} className="bg-white border border-gray-100 rounded-2xl p-4 flex justify-between items-center group hover:border-blue-200 transition-colors">
                              ...
                              <div className="text-right shrink-0 ml-2">
                                  <div className={`font-black text-sm ${activeIncomeDetail === 'FD' ? 'text-yellow-600' : 'text-blue-600'}`}>+${Math.round(item.monthly).toLocaleString()}</div>
          ...
          <div className="fixed inset-0 z-[70] flex items-center justify-center px-4">
              ...
              <div className="bg-white w-full max-w-xs rounded-[3rem] p-8 relative z-10 animate-in zoom-in-95 shadow-2xl">
                  ...
                  <div className="relative">
                      ...
                      <input type="number" autoFocus value={tempGoal} onChange={(e) => setTempGoal(e.target.value)} className="w-full bg-gray-50 p-6 pl-10 rounded-[2rem] text-3xl font-black text-blue-600 text-center outline-none ring-2 ring-transparent focus:ring-blue-100 transition-all" />
                  </div>
