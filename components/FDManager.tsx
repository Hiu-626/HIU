      ...
      {isAdding && (
          <div className="bg-white p-5 rounded-2xl shadow-xl border border-gray-100 mb-6 animate-in slide-in-from-top-4">
              <div className="flex bg-gray-100 p-1 rounded-xl mb-4">
                  <button onClick={() => setNewFD({...newFD, type: 'Fixed'})} className={`flex-1 py-2 text-xs font-bold rounded-lg ${newFD.type === 'Fixed' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-400'}`}>Fixed (External)</button>
                  <button onClick={() => setNewFD({...newFD, type: 'Savings'})} className={`flex-1 py-2 text-xs font-bold rounded-lg ${newFD.type === 'Savings' ? 'bg-white text-green-600 shadow-sm' : 'text-gray-400'}`}>Savings (Internal)</button>
              </div>
              <div className="space-y-4">
                  ...
                  <div className={`p-4 rounded-xl border ${newFD.type === 'Savings' ? 'bg-green-50 border-green-100' : 'bg-blue-50 border-blue-100'}`}>
                      <div className="flex justify-between text-[10px] font-bold text-gray-400 uppercase">
                          ...
                          {newFD.type === 'Savings' && <span className="text-green-600 flex items-center font-black"><EyeOff className="w-2 h-2 mr-1"/>Excluded from Net Worth</span>}
                      </div>
                      <div className="flex justify-between font-black mt-1">
                          <span className="text-green-600">+{estimation.interest.toLocaleString()}</span>
                          <span className="text-blue-700">{newFD.type === 'Savings' ? 'Interest credited later' : estimation.total.toLocaleString()}</span>
                      </div>
                  </div>
                  <button onClick={handleAdd} className="w-full bg-[#0052CC] text-white font-bold py-3.5 rounded-xl">Create Record</button>
              </div>
          </div>
      )}
                                ...
                                <div className="flex items-center gap-1.5 mb-0.5">
                                    ...
                                    {fd.type === 'Savings' && (
                                        <span className="bg-green-100 text-green-700 text-[9px] px-1.5 py-0.5 rounded font-black flex items-center">
                                            <EyeOff className="w-2 h-2 mr-1"/>Internal Fund (Excluded)
                                        </span>
                                    )}
                                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-white w-full max-w-sm rounded-3xl p-6 relative animate-in zoom-in-95">
                ...
                <div className="text-center mb-6">
                    ...
                    <h2 className="text-xl font-bold">Renew / Rollover</h2>
                    <p className="text-[11px] text-gray-400">Set interest to 0 for no compounding.</p>
                </div>
                <div className="bg-gray-50 p-4 rounded-2xl mb-6 space-y-2">
                    <div className="flex justify-between text-sm"><span>Current Principal</span><span className="font-bold">${rolloverTarget.principal.toLocaleString()}</span></div>
                    ...
                    <div className="border-t pt-2 flex justify-between font-bold text-[#0052CC]"><span>New Principal</span><span>${(rolloverTarget.principal + rolloverInterest).toLocaleString()}</span></div>
                </div>
                
                <div className="space-y-4 mb-6">
                    <input type="number" value={rolloverNewRate} onChange={(e) => setRolloverNewRate(Number(e.target.value))} className="w-full bg-gray-50 p-3 rounded-xl font-bold" placeholder="New Rate %" />
                ...
                <button onClick={confirmRollover} className="w-full bg-[#0052CC] text-white font-bold py-4 rounded-xl shadow-lg">Confirm Renewal</button>
            </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-white w-full max-w-sm rounded-3xl p-6 relative animate-in zoom-in-95">
                ...
                <div className="text-center mb-6">
                    ...
                    <h2 className="text-xl font-bold">Settle Interest</h2>
                </div>
                <div className="bg-gray-50 p-4 rounded-2xl mb-4 space-y-2">
                    <div className="flex justify-between text-sm text-gray-400"><span>Principal (Unchanged)</span><span>${settleTarget.principal.toLocaleString()}</span></div>
                    <div className="flex justify-between items-center text-sm text-green-600 font-bold">
                        <span>Realized Interest</span>
                    ...
                    <div className="border-t pt-2 flex justify-between font-bold text-orange-600">
                        <span>{settleTarget.type === 'Savings' ? 'Est. Credit' : 'Total Return'}</span>
                ...
                <div className={`p-3 rounded-xl text-[11px] mb-6 flex gap-2 ${settleTarget.type === 'Savings' ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-blue-50 text-blue-700 border border-blue-100'}`}>
                    ...
                    <span>
                        {settleTarget.type === 'Savings' 
                            ? "Savings Mode: Principal stays, only interest credited." 
                            : "Fixed Mode: Principal & Interest returned."}
                    </span>
                </div>

                <div className="mb-6">
                    <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">Credit Account</label>
                ...
                <button onClick={confirmSettle} className="w-full bg-orange-500 text-white font-bold py-4 rounded-xl shadow-lg">Confirm Settlement</button>
            </div>
        </div>
      )}
    </div>
